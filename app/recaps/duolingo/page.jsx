...
const translations = {
  ...
  es: {
    ...
    subtitle: "Destructor de metas. Competidor de la Liga Diamante. 8 cursos, 330K XP de por vida, y una racha de 152 días.",
    ...
    personalRecords: "Récords Personales", achievements: "Premios y Logros",
    behavioralPattern: "Patrón Conductual", contextNote: "Nota de Contexto",
    crossPlatform: "Nota Multiplataforma", showAll: "Mostrar Todo",
    resetLang: "Restablecer a Inglés", topTwoPercent: "Top 2% de todos",
    daysConsecutive: "días consecutivos", primaryLang: "Primario (2025)",
    weeksCompeting: "semanas compitiendo", sideQuest: "Misión secundaria",
    hours: "horas", maxed: "completado", new_: "nuevo",
  },
  ru: {
    flag: "🇷🇺", label: "Русский",
    pageTitle: "Duolingo 2025", topBadge: "Топ 2% в мире",
    subtitle: "Покоритель целей. Участник Алмазной лиги. 8 курсов, 330 тысяч XP за всё время, серия 152 дня.",
    totalXp: "Всего XP (2025)", minutes: "Минуты", streak: "Серия", italianScore: "Итал. балл",
    diamondLeague: "Алмазная лига", chessXp: "XP Шахматы",
    allTimeCourses: "Все курсы", duoQuotes: "Дуо сказал лучше всех",
    personalRecords: "Личные рекорды", achievements: "Награды и достижения",
    behavioralPattern: "Поведенческий паттерн", contextNote: "Контекст",
    crossPlatform: "Кросс-платформа", showAll: "Показать всё",
    resetLang: "Сбросить на английский", topTwoPercent: "Топ 2% всех",
    daysConsecutive: "дней подряд", primaryLang: "Основной (2025)",
    weeksCompeting: "недель", sideQuest: "Побочный квест",
    hours: "часов", maxed: "макс.", new_: "нов.",
  },
  de: {
    flag: "🇩🇪", label: "Deutsch",
    ...
    personalRecords: "Persönliche Rekorde", achievements: "Auszeichnungen",
    behavioralPattern: "Verhaltensmuster", contextNote: "Kontexthinweis",
    crossPlatform: "Plattformübergreifend", showAll: "Alle anzeigen",
    resetLang: "Auf Englisch zurücksetzen", topTwoPercent: "Top 2% aller",
    daysConsecutive: "aufeinanderfolgende Tage", primaryLang: "Primär (2025)",
  ...
  da: {
    ...
    subtitle: "Målknuser. Diamantliga-deltager. 8 kurser, 330K XP i alt, og en 152-dages stime.",
    ...
    personalRecords: "Personlige Rekorder", achievements: "Præmier og Præstationer",
    behavioralPattern: "Adfærdsmønster", contextNote: "Kontekstnote",
    crossPlatform: "Tværplatform Note", showAll: "Vis Alle",
    resetLang: "Nulstil til engelsk", topTwoPercent: "Top 2% af alle",
    daysConsecutive: "dage i træk", primaryLang: "Primær (2025)",
...
const courses = [
  { name: "Spanish", flag: "🇪🇸", xp: 113801, lang: "es", color: "#FF4B4B" },
  { name: "Russian", flag: "🇷🇺", xp: 113600, lang: "ru", color: "#1CB0F6" },
  { name: "German", flag: "🇩🇪", xp: 41294, lang: "de", color: "#FF9600" },
  { name: "Italian", flag: "🇮🇹", xp: 34724, lang: "it", color: "#58CC02" },
  { name: "English", flag: "🇺🇸", xp: 17850, lang: "en", color: "#1CB0F6" },
  { name: "Danish", flag: "🇩🇰", xp: 7430, lang: "da", color: "#FF4B4B" },
  { name: "Chess", flag: "♟️", xp: 1134, lang: null, color: C.teal },
  { name: "Math", flag: "🧮", xp: 262, lang: null, color: C.blue },
];
...
const duoQuotes = [
  { text: "Now I'm intimidated by YOU", context: "Diamond League — 8 weeks", color: C.blue },
...
@keyframes pulse{0%,100%{opacity:0.6}50%{opacity:1}}`}</style>
      ...
      <div style={{ padding: "40px 24px 32px", background: `linear-gradient(180deg, ${C.green}15 0%, transparent 70%)`, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 920, margin: "0 auto" }}>
          <div style={{ fontFamily: font.mono, fontSize: 9, color: C.green, letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: 10 }}>DDL · CONSOLE · YEAR-END RECAPS</div>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 6, flexWrap: "wrap" }}>
            ...
            {lang !== "en" && (
              <button onClick={() => setLang("en")} style={{ padding: "3px 10px", borderRadius: 12, border: `1px solid ${C.borderMed}`, background: C.card, fontFamily: font.mono, fontSize: 9, color: C.creamMid, cursor: "pointer", marginLeft: "auto" }}>🇺🇸 {t.resetLang}</button>
            )}
          </div>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "24px 24px 48px" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {[
            ...
            { label: t.minutes, value: "1,673", color: C.violet, sub: `≈ 27.9 ${t.hours}` },
        ...
        <div style={{ marginTop: 28, display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
          ...
          {lang !== "en" && <span style={{ fontFamily: font.mono, fontSize: 8, color: C.green, animation: "pulse 2s infinite" }}>● {translations[lang]?.label}</span>}
        </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    {isClickable && (
                      <span style={{ fontFamily: font.mono, fontSize: 7, padding: "1px 5px", borderRadius: 3, background: isActive ? c.color + "30" : C.creamGhost, color: isActive ? c.color : C.creamDim }}>
                        {isActive ? "● ACTIVE" : "click to translate"}
                      </span>
                    )}
        ...
        <div style={{ marginTop: 10, padding: "10px 14px", background: C.card, border: `1px solid ${C.border}`, borderRadius: 7 }}>
          ...
          <div style={{ fontFamily: font.body, fontSize: 12, color: C.creamMid, lineHeight: 1.6 }}>
            Spanish (113,801) and Russian (113,600) are separated by 201 XP — a 0.18% difference. Two entirely different alphabets, two entirely different language families, studied to virtually identical depth.
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {records.map(r => (
            <div key={r.label} style={{ flex: "1 1 160px", background: C.card, border: `1px solid ${r.manic ? C.amber + "15" : r.color + "20"}`, borderRadius: 7, padding: "16px", position: "relative", overflow: "hidden", opacity: r.manic ? 0.7 : 1 }}>
              ...
              {r.manic && <div style={{ fontFamily: font.mono, fontSize: 7, padding: "1px 5px", borderRadius: 2, background: C.amberDim, color: C.amber, display: "inline-block", marginBottom: 4 }}>MANIC EPISODE — ONE DAY</div>}
        ...
        <div style={{ marginTop: 10, padding: "12px 16px", background: C.card, border: `1px solid ${C.border}`, borderRadius: 7 }}>
          ...
          <div style={{ fontFamily: font.body, fontSize: 12, color: C.creamMid, lineHeight: 1.6 }}>
            The April 30, 2023 records — 14,648 XP and 156 perfect lessons — were logged in a single day during a manic episode. They are data, not benchmarks. The 2025 numbers (55,775 XP across the year, 152-day streak, top 2% globally) are what sustained, regulated engagement looks like. Less explosive, more durable. That's the real achievement.
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 8 }}>
          {displayAch.map(a => (
            <div key={a.name} style={{ background: C.card, border: `1px solid ${a.isNew ? a.color + "30" : C.border}`, borderRadius: 7, padding: "12px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ fontFamily: font.mono, fontSize: 9, color: C.green }}>{a.progress || "Earned"} ✓</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 28, padding: "18px 20px", background: C.green + "08", border: `1px solid ${C.green}25`, borderRadius: 9, borderLeft: `3px solid ${C.green}` }}>
          ...
          <div style={{ fontFamily: font.body, fontSize: 14, color: C.creamHigh, lineHeight: 1.7, marginBottom: 10 }}>
            Eight courses across six languages, chess, and math — but the real signal is the near-perfect parity between Spanish and Russian. 113,801 vs 113,600. Two completely different writing systems studied to identical depth. That's not dabbling. That's a controlled experiment with a sample size of one.
          </div>
          <div style={{ fontFamily: font.body, fontSize: 14, color: C.creamMid, lineHeight: 1.7 }}>
            Italian is the 2025 focus (primary language, Score 13), but it's fourth all-time behind Spanish, Russian, and German. The pattern: intense early investment across multiple fronts, then narrowing to one sustained practice. The same cycle that produced 44 DDL systems — broad exploration, then governed specialization. Chaos → Structured → Automated, applied to language acquisition.
          </div>
        </div>
