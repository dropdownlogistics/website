...
const wings = [
  ...
  {
    label: 'Story', color: C.crimson, routes: [
  ...
  {
    label: 'About', color: C.creamDim, routes: [
      { href: '/about', label: 'Operator Profile' },
      { href: '/social', label: 'Social Hub' },
    ],
  },
...
export default function SiteMap() {
  ...
  const filtered = wings.map(wing => ({
    ...wing,
    routes: filter
      ? wing.routes.filter(r =>
          r.label.toLowerCase().includes(filter.toLowerCase()) ||
          r.href.toLowerCase().includes(filter.toLowerCase())
        )
      : wing.routes,
  })).filter(wing => wing.routes.length > 0);
    ...
    <div style={{ background: C.navy, minHeight: '100vh', color: C.cream, fontFamily: font.body }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontFamily: font.mono, fontSize: 11, color: C.amber, letterSpacing: '0.15em', marginBottom: 12 }}>
            DDL &middot; SITE MAP
          </div>
          ...
          {filtered.map(wing => (
            <div key={wing.label} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid ' + C.border }}>
                <span style={{ fontFamily: font.display, fontWeight: 700, fontSize: '1rem', color: wing.color }}>
                  {wing.label}
                </span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                {wing.routes.map(route => (
                  <Link
                    ...
                    onMouseEnter={e => {
                      e.currentTarget.querySelector('.rl').style.color = wing.color;
                    }}
                    onMouseLeave={e => {
                      e.currentTarget.querySelector('.rl').style.color = C.creamMid;
                    }}
                  >
